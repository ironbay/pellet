...
func main() {
    ...
    for _, f := range files {
...

        {
            ...
            absolute := path.Join(output, o)
            ioutil.WriteFile(absolute, css.Bytes(), f.Mode())
            exec.Command("cleancss", "-o", absolute, absolute).Run()
            exec.Command("autoprefixer", absolute).Run()
